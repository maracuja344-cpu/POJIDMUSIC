...
const RELEASE_MARKER = `<meta name="pojidmusic-release" content="pwa-v88">`;
const SERVED_RELEASE_MARKER = `<meta name="pojidmusic-release" content="${RELEASE_VERSION}">`;
const ENTRY_VERSION = "88";
...
function versionNavigationHtml(html) {
    ...
    if (!next.includes("telegram-profile.css")) next = next.replace("</head>", `    <link rel="stylesheet" href="telegram-profile.css?v=${ENTRY_VERSION}">\n</head>`);
    ...
    if (!next.includes("queue-sheet-v88.css")) next = next.replace("</head>", `    <link rel="stylesheet" href="queue-sheet-v88.css?v=${ENTRY_VERSION}">\n</head>`);
    ...
    if (!next.includes("js/queue-sheet-motion.js")) next = next.replace("</body>", `    <script type="module" src="js/queue-sheet-motion.js?v=${ENTRY_VERSION}"></script>\n</body>`);
