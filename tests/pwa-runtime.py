...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="pojidmusic-pwa-runtime-") as profile:
        ...
        try:
            ...
            set_release("pwa-test-b")
            client.evaluate("navigator.serviceWorker.getRegistration().then((r) => r.update())")
            wait_for(client,
                "document.querySelector('meta[name=pojidmusic-release]')?.content === 'pwa-test-b' && "
                "window.__pwaServedScriptRelease === 'pwa-test-b'",
                "release B controller reload", timeout=40)
            wait_for(client,
                "document.querySelectorAll('#all-tracks .release-card').length > 0",
                "release B usable UI")
            release_b = client.evaluate(PAGE_STATE)
            assert_release(release_b, "pwa-test-b")
            assert release_b["loadCount"] == initial_load_count + 1, release_b
            assert release_b["reloadGuard"] == "pwa-test-b", release_b
            assert "pojidmusic-shell-pwa-test-b" in release_b["cacheNames"]
            ...
            assert_release(offline, "pwa-test-b")
