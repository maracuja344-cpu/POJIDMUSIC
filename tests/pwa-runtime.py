...
class ReleaseHandler(SUPPORT.QuietHandler):
    ...
    def do_GET(self):
        ...
        source = source.replace("pwa-v10", self.release)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="pojidmusic-pwa-runtime-") as profile:
        ...
        try:
            ...
            wait_for(client,
                "document.querySelector('meta[name=pojidmusic-release]')?.content === 'pwa-v10' && "
                "window.__pwaServedScriptRelease === 'pwa-v10'",
                "release B controller reload", timeout=40)
            wait_for(client,
                "document.querySelectorAll('#all-tracks .release-card').length > 0",
                "release B usable UI")
            release_b = client.evaluate(PAGE_STATE)
            assert_release(release_b, "pwa-v10")
            assert release_b["loadCount"] == initial_load_count + 1, release_b
            assert release_b["reloadGuard"] == "pwa-v10", release_b
            assert "pojidmusic-shell-pwa-v10" in release_b["cacheNames"]
            ...
            assert_release(offline, "pwa-v10")
