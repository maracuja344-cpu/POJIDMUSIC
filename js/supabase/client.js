...
function createTelegramRelinkModal() {
    if (document.querySelector("[data-telegram-relink-modal]")) {
        return document.querySelector("[data-telegram-relink-modal]");
    }
    ...
    modal.innerHTML = `
        ...
        <div class="auth-dialog">
            ...
            <p class="auth-message" data-telegram-relink-message>
                Войди в уже существующий POJIDMUSIC-аккаунт. Этот Telegram будет привязан к нему вместо временного профиля слушателя.
            </p>
...
function getTelegramRelinkMessage(error, status) {
    ...
    if (status === 401) {
        return "Telegram-сессия устарела. Закрой Mini App и открой его заново через бота.";
    }
    if (status === 409) {
        return "Этот аккаунт уже привязан к другому Telegram либо текущую привязку нельзя безопасно заменить.";
    }
...
async function initializeTelegramRelinkUi() {
    ...
    document.addEventListener("click", (event) => {
        if (event.target.closest("[data-telegram-relink-open]")) {
            event.preventDefault();
            openModal();
            return;
        }
        if (event.target.closest("[data-telegram-relink-close]")) {
            event.preventDefault();
            closeModal();
        }
    });
    ...
    document.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            const result = await requestTelegramAuth(
                "relink",
                initData,
            ...
            if (profile?.role === "artist") {
                ...
                if (artist?.slug) {
                    window.location.replace(
                        `${window.location.pathname}?artist=${encodeURIComponent(artist.slug)}`
                    );
                    return;
                }
            }
            ...
            message.textContent = getTelegramRelinkMessage(
                error,
                error?.status
            );
        } finally {
