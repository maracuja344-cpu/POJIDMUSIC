...
function publishAuthState(nextState) {
    ...
    authStateListeners.forEach((listener) => {
        try {
            ...
            console.error(
                "Не удалось обновить зависимый интерфейс Auth.",
                error
            );
        }
    });
...
function getAuthElements() {
    return {
        ...
        userControls:
            document.querySelector(".auth-user-controls"),
        profileButton:
            document.querySelector(".auth-profile-button"),
        profileMenu:
            document.querySelector(".profile-menu"),
        userIdentity:
            document.querySelector(".auth-user-identity"),
        profileNote:
            document.querySelector(".auth-profile-note"),
        signOutButton:
            document.querySelector(".auth-sign-out-button"),
        modal:
        ...
        loginPanel:
            document.querySelector("#auth-login-panel"),
        signupPanel:
            document.querySelector("#auth-signup-panel"),
        message:
...
function setProfileMenuOpen(
    ...
    if (focusFirstItem) {
        elements.profileMenu.querySelector(
...
function setUserIdentity(elements, identity) {
    ...
    elements.profileButton.setAttribute(
        "aria-label",
        `Открыть меню профиля: ${safeIdentity}`
    );
}
...
function renderHeaderAuthState(elements, session) {
    ...
    elements.openButton.hidden = isAuthenticated;
    elements.userControls.hidden = !isAuthenticated;
...
async function refreshProfile(elements, user, session) {
    ...
    интерфейс от задержки репликации/API. Клиент ничего не вставляет.
    */
    for (
        let attempt = 0;
        result.state === "missing" && attempt < 2;
        attempt += 1
    ) {
...
async function handleLoginSubmit(event, elements) {
    ...
    try {
        ...
        if (error || !data.user || !data.session) {
            setMessage(
                elements,
                getReadableAuthError(error, "login"),
                "error"
            );
            return;
        }
...
async function handleSignupSubmit(event, elements) {
    ...
    try {
        ...
        if (error || !data.user) {
            setMessage(
                elements,
                getReadableAuthError(error, "signup"),
                "error"
            );
            return;
        }
...
async function handleSignOut(elements) {
    ...
    try {
        ...
        if (error) {
            elements.profileNote.textContent =
                getReadableAuthError(error, "signout");
            elements.profileNote.hidden = false;
            return;
        }
        ...
        elements.profileNote.textContent =
            "Не удалось выйти. Проверьте соединение.";
        elements.profileNote.hidden = false;
    } finally {
...
export function initializeAuth() {
    ...
    document.addEventListener("click", (event) => {
        if (
            elements.profileMenu.hidden ||
            elements.userControls.contains(event.target)
        ) {
            return;
        }
    ...
    elements.userControls.addEventListener(
        ...
        () => {
            window.setTimeout(() => {
                if (
                    !elements.userControls.contains(
                        document.activeElement
                    )
                ) {
                    setProfileMenuOpen(elements, false);
    ...
    elements.loginForm.addEventListener(
        "submit",
        (event) => {
            void handleLoginSubmit(event, elements);
        }
    );
    ...
    elements.signupForm.addEventListener(
        "submit",
        (event) => {
            void handleSignupSubmit(event, elements);
        }
    );
